...
<style>
  ...
  #trippie .t-launch .t-face{width:26px; height:26px; display:grid; place-items:center; background:rgba(255,255,255,0.22); border-radius:50%;}
  #trippie .t-launch svg{width:16px; height:16px;}
  #trippie .t-panel{
  ...
  #trippie .t-head{display:flex; align-items:center; gap:10px; padding:14px 14px; color:#fff;
    background:linear-gradient(120deg,#C22A66,#8E3A73 55%,#6E54A6);}
  #trippie .t-head .t-av{width:34px; height:34px; border-radius:50%; background:rgba(255,255,255,0.22); display:grid; place-items:center;}
  #trippie .t-head svg{width:20px; height:20px;}
  ...
  #trippie .t-body{flex:1; overflow-y:auto; padding:14px; background:#FFF8FA; display:flex; flex-direction:column; gap:10px;}
  ...
  #trippie .t-msg.bot{align-self:flex-start; background:#fff; border:1px solid rgba(36,30,35,0.1); color:#241E23; border-bottom-left-radius:4px;}
  ...
  @media (prefers-reduced-motion: reduce){ #trippie .t-launch, #trippie .t-panel{animation:none;} #trippie .t-typing i{animation:none;} }
</style>

@php
  $t_plane = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 2 11 13"/><path d="M22 2 15 22l-4-9-9-4 20-7Z"/></svg>';
@endphp

<div id="trippie" aria-live="polite">
  <button class="t-launch" type="button" data-t-open>
    <span class="t-face">{!! $t_plane !!}</span> Ask Trippie
  </button>

  <div class="t-panel" role="dialog" aria-label="Chat with Trippie">
    <div class="t-head">
      <span class="t-av">{!! $t_plane !!}</span>
...
(function () {
  ...
  function add(role, text) {
    ...
    d.className = 't-msg ' + (role === 'me' ? 'me' : 'bot');
    d.innerHTML = role === 'me' ? text.replace(/[&<>]/g, function (c) { return { '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c]; }) : fmt(text);
  ...
  history.forEach(function (h) { add(h.role === 'model' ? 'bot' : 'me', h.text); });
  ...
  function ask(message) {
    ...
    fetch('/trippie', {
      ...
      .then(function (r) { return r.ok ? r.json() : { reply: r.status === 429 ? "Whoa, slow down a sec — ask me again in a moment 😄" : "I couldn't reach my maps just now — try again shortly." }; })
      .then(function (d) {
        typing(false);
        add('bot', d.reply);
      ...
      .catch(function () { typing(false); add('bot', "Hmm, connection hiccup — try that again? 🧭"); })
